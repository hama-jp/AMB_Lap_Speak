...
- 各マシンについて、重要なパフォーマンス指標が表示されます:
  - **Ponder**: マシンの番号。クリックすると詳細な履歴ページに移動します。
  - **Best Lap**: サーバー起動後の最速ラップ。
  - **Avg (10 Laps)**: 直近10周の移動平均タイム。
  - **Std Dev**: ラップタイムの標準偏差。タイムのばらつき（安定性）を示します。
  - **Latest Lap**: 直前に計測されたラップタイム。
  - **Last Pass**: 最後にコントロールラインを通過した時刻。
  - **Announce**: そのマシンの音声読み上げを有効/無効にするチェックボックス。
...
- 有効にすると、マシンがコントロールラインを通過した際に、そのマシンの番号とラップタイムを読み上げます。
- **読み上げ例**: 「ポンダー 5、32.15」
...
データベースを含めたシステム全体を最も簡単に起動する方法です。

1.  **デコーダーのIPアドレスを設定:**
    `docker-compose.yml` ファイルを開き、`client` サービスの `AMB_IP` 環境変数を、お使いのAMB P3デコーダーのIPアドレスに設定します。
...
2.  **システムの起動:**
    ...
    - MySQLデータベースのコンテナを起動します。
    - `app` (Webサーバー) と `client` (データ収集) のコンテナを起動します。
    - データベーススキーマを自動的に作成します。
